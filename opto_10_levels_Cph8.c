...
gsl_vector* field_func(const gsl_vector *A_phi_params, parameters *params)
{
	...
	for(i = 0; i<Tsteps; i++)
	{
		...
		double gaussian = gsl_sf_exp(-(t_i-500.)*(t_i-500.)/(50.*50.));
		// printf("%d %3.4f \n", i, gaussian);

		for (j = 0; j < Nfreq; j++) 
		{
			double x = gsl_vector_get(A_phi_params, j)*gsl_sf_cos(gsl_vector_get(params->frequency, 0)*t_i);
			result += x;
    		}
...
int main(int argc, char *argv[])
{
	...
	double gamma_3_5  = 1./(freq_central*68.5*1000.);	// 68.5 ps
	double gamma_3_4  = 2.5/(freq_central*68.5*1000.);	// 68.5 / 2.5 ps
	double gamma_5_6  = 1./(freq_central*10.);		// 10 fs

	double gamma_9_6  = 1./(freq_central*50.);		// 50 fs
	double gamma_7_8  = 1./(freq_central*50.);     	 	// 50 fs
	double gamma_8_10 = 1./(freq_central*2.5*1000.);	// 2.5 ps
	double gamma_8_9  = 1./(freq_central*500.);		// 500 fs
	double gamma_10_1 = 1./(freq_central*10.);		// 10 fs
	...
	const double sigma2 = (19.1538/1.6651)*(19.1538/1.6651);	// in fs from TBWP relation
